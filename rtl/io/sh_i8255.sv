...
//   - the strobed OUTPUT handshake on group A, engaged by mode 1 output
//     (control bits 6:5 = 01, A out) or mode 2 (bit 6 = 1, bidirectional —
//     hangon programs 0xC0 and uses only the output half): the main PPI's
//     port A is the sound latch, /OBFa on PC7 is the Z80's NMI (active
//     low), the Z80's latch read drives /ACKa, INTEa via BSR of PC6,
//     INTRa on PC3. A port C read returns the status bits in place of
//     PC7/PC6/PC3. Mode 2's input half (/STB, IBF) is not modelled: no
//     game on this board reads port A through it.
...
reg       mode1_a;                 // group A strobed-output handshake engaged
reg       mode2_a;                 // engaged by mode 2 (bidirectional)
...
// read view. Mode-2 status: PC5 = IBF (always 0 here: the input half has
// no writer — the Z80 side has no path to strobe data in, in MAME or on
// this board), PC4 = INTE2 (its BSR flip-flop). Without this, hangon's
// main CPU saw a phantom IBF = 1 from the input pins, read a "response"
// from port A, and its sound sequencer walked off into the weeds.
wire [7:0] rd_c_pins = {dir_cu ? in_c[7:4] : latch_c[7:4], dir_cl ? in_c[3:0] : latch_c[3:0]};
wire [7:0] rd_c = mode2_a ? {obf_n, inte, 1'b0, latch_c[4], intr, rd_c_pins[2:0]} :
                  mode1_a ? {obf_n, inte, rd_c_pins[5:4], intr, rd_c_pins[2:0]} : rd_c_pins;

always @* begin
    case (addr)
        2'd0: dout = mode2_a ? 8'h00 : (dir_a ? in_a : latch_a);   // mode 2: empty input buffer
...
always @(posedge clk) begin
    ...
    if (reset) begin
        ...
        mode1_a <= 1'b0; mode2_a <= 1'b0;
        dir_a <= 1'b1; dir_b <= 1'b1; dir_cl <= 1'b1; dir_cu <= 1'b1;
            ...
            case (addr)
                ...
                2'd3: begin
                    if (din[7]) begin
                        ...
                        mode1_a <= ((din[6:5] == 2'b01) && !din[4]) || din[6];
                        mode2_a <= din[6];
                        dir_a   <= din[4];
                        dir_cu  <= din[3];
                        dir_b   <= din[1];

...
releases/Arcade-SegaSpaceHarrier_<date>.rbf as _Arcade/cores/<file>, with MD5 and
...
def main():
    ...
    ap.add_argument("--repo", default="rossops/Arcade-SegaSpaceHarrier_MiSTer")
    ...
    for mra in sorted(glob.glob(os.path.join(rel_dir, "**", "*.mra"), recursive=True)):
        ...
        while d:
            folders[f"_Arcade/{d}"] = {}
            d = os.path.dirname(d)
    rbfs = sorted(glob.glob(os.path.join(ROOT, "releases", "Arcade-SegaSpaceHarrier_*.rbf")),
                  key=lambda p: re.search(r"_(\d{8})\.rbf$", p).group(1))
    if not rbfs:
        raise SystemExit("no releases/Arcade-SegaSpaceHarrier_<date>.rbf")
    rbf = rbfs[-1]
